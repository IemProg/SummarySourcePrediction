...
os.makedirs("saved_data_tokenz_1024", exist_ok=True)
output_path1 = os.path.join(os.getcwd(), "saved_data_tokenz_512")
output_path = os.path.join(os.getcwd(), "saved_data_tokenz_1024")

save_np = True

path = os.path.join(output_path1, "X_train.json")
X_train = pd.read_json(path)
path = os.path.join(output_path1, "X_val.json")
X_val = pd.read_json(path)
path = os.path.join(output_path1, "X_test.json")  # This one is evaluation
X_test = pd.read_json(path)

# Initialize tokenizer.
tokenizer = RobertaTokenizer.from_pretrained("roberta-base", model_max_len=1024, truncation=True, padding=True)
...
output_path = os.path.join(os.getcwd(), "saved_data_tokenz_512")

if save_np:
    y_train, X_train_sum = tokenize_df(tokenizer, X_train, max_len = 1024, col = "summary")
    y_train, X_train_doc = tokenize_df(tokenizer, X_train, max_len = 512, col = "document")
    print("\t\t training dset has been tokenized !!!")
    y_val, X_val_sum = tokenize_df(tokenizer, X_val, max_len = 1024, col = "summary")
    y_val, X_val_doc = tokenize_df(tokenizer, X_val, max_len = 512, col = "document")
    print("\t\t validation dset has been tokenized !!!")
    y_test, X_test_sum = tokenize_df(tokenizer, X_test, max_len = 1024, col = "summary")
    y_test, X_test_doc = tokenize_df(tokenizer, X_test, max_len = 512, col = "document")
...
train_iter = DataLoader(dataset=dataset, batch_size=2, shuffle=True, num_workers=4)
#
...
valid_iter = DataLoader(dataset=dataset, batch_size=2, shuffle=True, num_workers=4)
#
...
test_iter = DataLoader(dataset=dataset, batch_size=2, shuffle=False, num_workers=4)
...
scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=steps_per_epoch*1,num_training_steps=steps_per_epoch*NUM_EPOCHS)
        ...
        for batch in train_iter:
            ...
            if global_step % valid_period == 0:
                ...
                if best_valid_loss > valid_loss:
                    ...
                    save_path = output_path + '/model_both_WO_reg_512.pkl'
                    save_checkpoint(save_path, model, best_valid_loss)
                    save_path = output_path + '/metric_both_WO_reg_512.pkl'
...
train_loss_list, valid_loss_list, global_steps_list = load_metrics(output_path + '/metric_both_WO_reg_1024.pkl', device)
plt.plot(global_steps_list, train_loss_list, label='Train')
plt.plot(global_steps_list, valid_loss_list, label='Valid')
plt.xlabel('Global Steps', fontsize=14)
plt.ylabel('Loss', fontsize=14)
plt.legend(fontsize=14)
plt.savefig('loss_both_WO_reg_1024.png', dpi=500)
...
save_path = output_path + '/model_both_WO_reg_1024.pkl'
...
#if save_np:
_, X_eval_sum = tokenize_df(tokenizer, X_eval, max_len = 1024, col = "summary", labels = False)
_, X_eval_doc = tokenize_df(tokenizer, X_eval, max_len = 512, col = "document", labels = False)
print("\t\t training dset has been tokenized !!!")

#save to npy file
ids, att = save_feats(X_eval_sum)
save_path = os.path.join(output_path, 'X_eval_sum_ids.npy')
np.save(save_path, ids)
save_path = os.path.join(output_path, 'X_eval_sum_att.npy')
np.save(save_path, att)

# save to npy file
ids, att = save_feats(X_eval_doc)
save_path = os.path.join(output_path, 'X_eval_doc_ids.npy')
np.save(save_path, ids)
save_path = os.path.join(output_path, 'X_eval_doc_att.npy')
np.save(save_path, att)

X_eval_ids_sum = np.load(output_path + "/" + 'X_eval_sum_ids.npy')
X_eval_att_sum = np.load(output_path + "/" + "X_eval_sum_att.npy")
X_eval_ids_doc = np.load(output_path + "/" + 'X_eval_doc_ids.npy')
X_eval_att_doc = np.load(output_path + "/" + "X_eval_doc_att.npy")
...
eval_iter = DataLoader(dataset=dataset, batch_size=2, shuffle=False, num_workers=4)
...
with open("submission_RobertaImad_BothSumDoc_1024_WO_regularized.csv", "w") as pred:
