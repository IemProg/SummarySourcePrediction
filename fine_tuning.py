...
def tokenize_sentence_preprocess_function(examples):
    return tokenizer(examples["tokens"], truncation=True, padding='max_length', max_length=200,
...
def main():
    ...
    dataset = Dataset.from_pandas(training_set).train_test_split(train_size=0.95)
    ...
    with open("submission_distilert_.csv", "w") as pred:
